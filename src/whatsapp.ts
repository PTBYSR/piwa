import { default as makeWASocket, useMultiFileAuthState, DisconnectReason, WASocket, fetchLatestBaileysVersion, Browsers } from '@whiskeysockets/baileys';
...
export async function sendPresence(jid: string, presence: 'composing' | 'paused' | 'available' | 'unavailable'): Promise<void> {
  if (currentSock) {
